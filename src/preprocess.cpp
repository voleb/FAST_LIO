...
// livox 계열 단일화(2026-09-11): Ericsii 계보는 avia 를 구형 인터페이스 패키지로
// 받았으나 CustomMsg 스키마가 동일해 livox_ros_driver2 로 통일했다.
void Preprocess::process(const livox_ros_driver2::msg::CustomMsg::UniquePtr &msg, PointCloudXYZI::Ptr& pcl_out)
{
  if (lidar_type == MID360) mid360_handler(msg);
  else                      avia_handler(msg);
  *pcl_out = pl_surf;
}
